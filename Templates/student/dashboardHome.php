...
if (isset($_SESSION['stuid'])) {
    // accessed collections from the database 
    ...
    if (isset($_GET["clickedfolder"])) {
        ...
        $parentFolderName = $_GET["clickedfolder_name"];
        $_SESSION['path'] = $_SESSION['path'] . "-" . $parentFolderId;
        $_SESSION['pathNames'] = $_SESSION['pathNames'] . "-" . $parentFolderName;
        $_SESSION['parent_id'] = new MongoDB\BSON\ObjectID($parentFolderId);
    }

    // for navigating to previous folders in path . Here also variables will be updated
    if (isset($_GET['previous'])) {
        $parentFolderId = $_GET["previous_id"];
        $path = explode("-", $_SESSION['path']);
        $pathNames = explode("-", $_SESSION['pathNames']);
        $tmp_path = array();
        $tmp_pathNames = array();
        foreach ($path as $key => $value) {
            array_push($tmp_path, $value);
            array_push($tmp_pathNames, $pathNames[$key]);
            if ($value == $parentFolderId) {
                break;
            }
        }
        $_SESSION['path'] = join("-", $tmp_path);
        $_SESSION['pathNames'] = join("-", $tmp_pathNames);
        $_SESSION['parent_id'] = new MongoDB\BSON\ObjectID($parentFolderId);
    }

    // if we came here after clicking on folder
    ...
    // if we came here for the first time
    else{
        $clg_id = $_SESSION["clg_id"];
        $_SESSION['path'] = $_SESSION['accessId'];
        $_SESSION['pathNames'] = "Home";
        $_SESSION['parent_id'] = new MongoDB\BSON\ObjectID($_SESSION['accessId']);
        $parentFolderId=new MongoDB\BSON\ObjectID($_SESSION['accessId']);
        $path = explode("-", $_SESSION['path']);
        $pathNames = explode("-", $_SESSION['pathNames']);
    }
?>
...
<head>
    ...
    <link rel="stylesheet" href="../../CSS/Folder/createQuiz.css">
...
<body>
    ...
    <div class="wrapper">
        ...
        ?>
        <div class="content">
            <div class="navContainer" style="margin: 3vh 2vw;">
                <?php
                include "./foldernavigation.php";
                ?>
            </div>

            <div class="folderContainer" style="z-index: 10;">
                <?php
                include "./displayFolder.php";
                ?>
            </div>
            <div class="accounts" id="accounts">
                ...
                <div class="c" style="font-size: 20px;">
                    <label>NUMBER OF STUDENTS<br>
                        COUNT:<?php
                                $stu_count = count(iterator_to_array($studentAccounts->find(["accessId" => (string)$parentFolderId])));
                                echo $stu_count;
                                ?><br><br>
                        NUMBER OF TEACHERS<br>
                        COUNT:<?php
                                $teacher_count = count(iterator_to_array($teacherAccounts->find(["accessId" => (string)$parentFolderId])));
                                echo $teacher_count;
                                ?>
                    </label>
                </div>
